...
def main():
    interf = BTinterface()
    path_len=int(input("enter the maximum path length"))
    maze = mz.Maze("big_maze_111.csv",6)
    maze.deadenddis_ini()
    ...
    s=[]
    best_strategy=0
    maze.short_path_initial()
    for i in range(len(maze.selection_score)-1,-1,-1):
        if(path_len>=maze.shortest_path[maze.selection_score[i][1]]):
            best_strategy=maze.selection_score[i][1]
            break
    for i in range(len(maze.deadends)):
        if(ans>maze.dis_dp(best_strategy,i)):
            index=i
            ans=maze.dis_dp(best_strategy,i)
    #for i in range(len(maze.deadends)):
        #print(maze.trace_path[2**len(maze.deadends)-1][index][i])
    first=maze.BFS_2(maze.nodes[5],maze.nodes[maze.trace_path[best_strategy][index][0]-1])
    for i in range(len(first)-1):
        s.append(first[i])
    for i in range(len(maze.deadends)-1):
        if(maze.trace_path[best_strategy][index][i+1]==999):
            break
        n_a=maze.nodes[maze.trace_path[best_strategy][index][i]-1]
        n_b=maze.nodes[maze.trace_path[best_strategy][index][i+1]-1]
        s.append(n_a)
