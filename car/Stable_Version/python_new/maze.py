...
class Maze:
    ...
    def getActions(self, nodes):
        ...
        return actions
    def short_path_initial(self):
        self.shortest_path=np.full(2**len(self.deadends),999)
        for i in range(2**len(self.deadends)):
            for j in range(len(self.deadends)):
                self.shortest_path[i]=min(self.shortest_path[i],self.dis_dp(i,j))
        return None
